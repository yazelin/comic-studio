...
5. **排版（三層合成）**：底圖（不動）→ **效果層** → **字層**。
   - 字層：對白氣泡是 HTML 文字疊在圖上（點擊新增、拖曳移動、雙擊改字），改字零成本；三層聲音＋效果字四型；寬度與字級可留自動（隨格寬縮放）或手動指定（單位＝格寬 %，跨裝置等比）；對白泡可選尾巴方向（上下左右，預設朝下，也可關掉）。工作台的氣泡跟匯出站用同一支字型、同一組內距行高字距，所見即所得。
