...
6. **匯出**：出版動作。每格拿「選定圖＋排版文字」請生圖模型 image-edit **整張重繪，把文字畫進圖**（對白＝白泡、內心＝無框浮字、旁白＝深底條、效果字＝畫進動作裡；泡形與精確擺位由模型照畫面決定）。字體自動統一：本章第一個烙好的格會當「字體錨」附進之後每格的參考圖，基準字感一致；效果字刻意破格、不吃基準。烙完逐格檢查錯字，不滿意單格重繪；成品存進章節的 `匯出/`。「匯出電子書」時已重繪的格用重繪圖，沒重繪的退回疊字，寫進 `dist/`。
