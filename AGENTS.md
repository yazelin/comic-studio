...
- `world/<id>/card.json` + `ref.png`=**世界風土庫**(場景、道具、風土素材:公會大廳、告示板、貨幣、文字筆跡、街市攤位…)。跟角色庫同形狀,分鏡用 `panel.world: ["node_guild_hall"]` 指名,生圖時當這一格的**場景鎖**附上去——沒有它,同一個大廳每格都會重抽一個樣子
  - **機位在工作台的「世界」頁用點的**:每張卡列出所有機位,按「在圖上設定」→ 平面圖上點兩下(位置、朝向),
    會寫進 `camera_geo` 並產出 `plan-<代號>.png`(只畫這個機位 + 視野扇形)。生圖時附這張——
    共用一張平面圖時上面好幾個機位標記擠在一起,模型得自己認「我是哪一個」,實測會認錯
  ...
  - `card.json` 的 `seating`(字串,可選)=**固定站位/座位**。平面圖鎖「家具在哪裡」、機位鎖「從哪裡看」,這一條鎖「**誰在哪裡**」——少了它,同一場戲每個人的位子會每格重排,讀者會覺得大家一直在換位子。寫死誰坐哪一側、從門數來第幾個
...
- `chapters/<n>/chapter.json` 的 `scenes`=**場次表**(影劇的「場」:同一地點、同一段連續時間的一串鏡頭)。分鏡表只描述單一鏡頭;場景、走位、道具、時間這些整場共用的東西寫在場次上,每格填 `scene_id` 自動繼承——**不要逐格手抄,抄漏一格就是連戲事故**
  - 欄位:`id`/`name`/`time`/`world`(整場預設)/`camera_default`/`state`(這場戲的世界狀態)/`blocking`(走位,整場不變)/`props`(誰身上有什麼)/`changes`
  - `changes: [{at:"p64", note:"石柱與土堤都不見了"}]` = **這場戲中途世界變了**。`at` 那格之後的每一格都會自動帶上那句話。實測事故:整場最重的三格畫著已經被切掉的石柱,因為「世界變了」只寫在發生的那一格裡
