...
export function buildStoryboardPrompt(chapterText, characters = []) {
  ...
  return [
    ...
    '- 動作欄**不要寫比喻**——scene 會逐字餵給生圖模型,比喻裡的名詞會被畫成真的道具(實測「像把一份報告闔上」生出木桌加一疊紙)。近景與特寫也要交代背景是什麼,否則模型會自己補一面牆。',
    '- **動作要連戲**:上一格結束的姿勢就是這一格的起點。同一場景的連續格之間,身體要有可信的位移或變化(至少重心、手、視線其中一項不同);真的要重複的格(刻意的定鏡)才寫成一模一樣。',
    '- 承接前一格的格,把前一格的 id 填進 `continues`——生圖時會把前一格的成品當參考圖,機位、場景與光線才會連得起來。**同一個人接下去才叫承接姿勢;換人的定鏡重複格(例如考生一個一個上場)照樣可以填,人物會以本格的 characters 為準。**',
    '- 台詞從原文取材,可精簡,不可改變劇情。',
    '- 12 到 30 格之間,依內容長度決定。',
    '',
    roster,
...
export function buildPanelPrompt({ style, panel, characterCards = [], worldCards = [], rules = [] }) {
  ...
  return [
    ...
    // 承前格那張同時帶著場景與「上一格那個人」。同一個人接下去時這是連戲;
    // 換人時(定鏡重複格最常見:三個考生依序上場)不切開語意的話,三個人會長成同一個。
    (panel.continues ? '第一張參考圖是上一格的成品:**機位、場景、光線照它接下去**。'
      + '畫面裡的人以本格指定的出場角色為準——**如果人換了,不要沿用上一格那個人的長相**,'
      + '只有同一個角色才承接姿勢。' : ''),
